...
\### Авторизация Обратите внимание, что для работы с API Wordstat нужен
отдельный токен. Тот, который используется для Директа, не подходит.
Чтобы получить токен для Wordstat нужно отправить заявку - [тут подробно
