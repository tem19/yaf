...
качестве конверсий  
`atribution` - модель атрибуции, одна либо несколько  
`filter` - фильтрация данных в отчете. Функция в качестве фильра ожидает
строку в формате “ПОЛЕ ОПЕРАТОР ЗНАЧЕНИЕ”. Подробнее о операторах и
полях, к которым они применимы, можно прочитать в справке:
<https://yandex.ru/dev/direct/doc/ru/filters>  
`search_query_report` - логическое значение, по умолчанию - `FALSE`;
если `TRUE`, то тип отчета меняется на SEARCH_QUERY_REPORT (предназначен
для выгрузки статистики по поисковым запросам)  
`numeric_fields_as_numeric` - логическое значение, по умолчанию -
`FALSE`, если `TRUE`, то для числовых значений не будет производится
корректировка типа.
